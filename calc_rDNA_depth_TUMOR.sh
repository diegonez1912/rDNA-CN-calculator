...
PROJECT_ID="$2" # Second command line argument (project name, e.g. GBM_TUMOR) used to organize output directories
PLOIDY_FILE="$3" # NEW --> Path to the specific ploidy TSV (e.g. scripts/ploidy_files/GBM/GBM_ploidy.tsv)
...
########################################
# Step 4: Ploidy correction for tumor samples
########################################

# Define default R values for ploidy correction (assuming diploid, or any other problem in ploidy data retrieval)
R_5S=1
R_45S=1

# Apply correction only if it is a tumor sample (TCGA code 01-09) and if the ploidy file exists
if [[ "${SAMPLE}" =~ -(0[1-9])[A-Z] ]]; then
  if [[ -f "${PLOIDY_FILE}" ]]; then
    echo "[INFO] Detected tumor sample with tissue code ${PROJECT_ID}. Applying ploidy correction."
    MATCH_ID=$(echo "${SAMPLE}" | cut -c 1-16) # Extract the first 15 characters to match with the ploidy file (e.g. TCGA-XX-XXXX-XXA)

    PLOIDY_DATA=$(grep -m 1 "${MATCH_ID}" "${PLOIDY_FILE}") # Get ploidy data for the sample (MATCH_ID) from the ploidy file, and stop after the first match (-m 1)

    if [[ -n "${PLOIDY_DATA}" ]]; then # Check if ploidy data was found for the sample
      echo "[INFO] Found ploidy data for ${SAMPLE}: ${PLOIDY_DATA}"    
      C_45=$(echo "${PLOIDY_DATA}" | awk '{print $3}') # Extract the ploidy value for 45S from the ploidy data (3rd column)
      C_5=$(echo "${PLOIDY_DATA}" | awk '{print $4}')  # Extract the ploidy value for 5S from the ploidy data (4th column)

      # Force them into decimal format so 'bc' doesn't crash
      # %.8f converts scientific notation (e-05) to 0.00000000
      C_45=$(printf "%.8f" "$C_45")
      C_5=$(printf "%.8f" "$C_5")
      
      # Calculate R values for ploidy correction --> R = (2 + C) / 2, where C is the ploidy value for the respective rDNA type
        # If C=0 (diploid), then (2+0)/2 = 1.0
        # If C=1 (one extra copy), then (2+1)/2 = 1.5
      R_45S=$(echo "scale=4; (2 + ${C_45}) / 2" | bc) # Calculate R for 45S with 4 decimals (using bc)
      R_5S=$(echo "scale=4; (2 + ${C_5}) / 2" | bc)   # Calculate R for 5S with 4 decimals (using bc)
    else
      echo "[WARNING] Sample ${MATCH_ID} not found in ${PLOIDY_FILE}. R=1"
    fi
  else
    echo "[INFO] Ploidy file ${PLOIDY_FILE} not found. Proceeding without ploidy correction (R=1)."
  fi
fi

echo "[INFO] R_5S: ${R_5S}"
echo "[INFO] R_45S: ${R_45S}"


########################################
# Step 5: Get position wise BRD for rDNA aligned reads (from step 2)
########################################
...
# 5S --> Normalize by BRD from chr1 single-copy regions
  ...
  # awk --> Normalize the background read depth at that position in the sample
    # We do this by dividing the raw number of reads at that position ($3) with the multiplication between: mean BRD value from single-copy regions on chr1 (BRD_CHR1) and ploidy correction factor (R_5S)
samtools depth -a -r "${RDNA_5S_CONTIG}" "${RDNA_BAM}" \
  | awk -v brd="${BRD_CHR1}" -v r="${R_5S}" 'BEGIN{OFS="\t"} {print $1, $2, $3/(brd*r)}' \
...
samtools depth -a -r "${RDNA_45S_CONTIG}" "${RDNA_BAM}" \
  | awk -v brd="${BRD_OTHER}" -v r="${R_45S}" 'BEGIN{OFS="\t"} {print $1, $2, $3/(brd*r)}' \
